...
# flux-framework.org/slurm-operator-bundle:$VERSION and flux-framework.org/slurm-operator-catalog:$VERSION.
IMAGE_TAG_BASE ?= flux-framework.org/slurm-operator
...
deploy-local: manifests kustomize build
	kubectl delete -f examples/dist/slurm-operator-local.yaml || true
	docker build -t ${DEVIMG} .
	cd config/manager && $(KUSTOMIZE) edit set image controller=${DEVIMG}
	$(KUSTOMIZE) build config/default > examples/dist/slurm-operator-local.yaml
	sed -i 's/        imagePullPolicy: Always/        imagePullPolicy: Never/' examples/dist/slurm-operator-local.yaml
...
build-config: manifests kustomize ## Deploy controller to the K8s cluster specified in ~/.kube/config.
	cd config/manager && $(KUSTOMIZE) edit set image controller=${IMG}
	$(KUSTOMIZE) build config/default > examples/dist/slurm-operator.yaml
...
.PHONY: clean
clean:
	kubectl delete -n slurm-operator svc --all --grace-period=0 --force
	kubectl delete -n slurm-operator cm --all --grace-period=0 --force
	kubectl delete -n slurm-operator pods --all --grace-period=0 --force
	kubectl delete -n slurm-operator jobs --all --grace-period=0 --force
	kubectl delete -n slurm-operator jobset --all --grace-period=0 --force
	kubectl delete -n slurm-operator slurm --all --grace-period=0 --force
